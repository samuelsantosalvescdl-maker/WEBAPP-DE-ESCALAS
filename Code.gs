...
function buildWeeklyHtml_(payload, options) {
  ...
  const statusClassMap = {
    'Folga': 'status-folga',
    'Abatimento': 'status-abatimento',
    'Dom. mês': 'status-dom-mes',
    'Licença': 'status-licenca',
  ...
  for (let start = 1; start <= daysInMonth; start += 7) {
    ...
    for (let d = start; d <= end; d++) out += '<th class="sub-head day-start">Entrada</th><th class="sub-head">Intervalo</th><th class="sub-head day-end">Saída</th>';
    out += '</tr></thead><tbody>';
